...
movement directions at the moment of correct target shown. It computes the cross-condition variance
for both the generalization of the space and the distractors encoded.
"""
...
def compute_cross_condition_variance_array(projected_mua, labels, model):
    """
    It computes the cross-condition variance for the first two dimensions of the model, given
    the projected data, the model (to normalize by the variance explain) and the parameter to
    check what is the trial-split.
    :param projected_mua: array of shape (n_trials, n_dims) with the projected data
    :param labels: vector with the conditions (n_trials,)
    :param model: trained model to get back the variance explained
    :return:
    """
    df = pd.DataFrame(projected_mua, columns=['dim1', 'dim2'])
    df['label'] = labels

    # Compute mean per condition
    grouped = df.groupby('label')[['dim1', 'dim2']].mean()

    # Variance across condition means
    var_across_means = grouped.var().values  # shape (2,)

    # Normalize weights to sum to 1
    weights = np.array(model.explained_variance_ratio_[:2])
    weights /= weights.sum()

    # Weighted sum
    weighted_cross_cond_var = (var_across_means * weights).sum()

    return weighted_cross_cond_var
        ...
        for probe in probes:
            ...
            for event_train in events_train:
                ...
                for bt in range(100):
                    ...
                    for distractor_test in events_test:
                        trial_type_test = int(distractor_test[-1])
                        if distractor_test == event_train:
                            continue
                        else:
                            behavior_distractor = f'{event_train}_dir'
                            ...
                            mua_test_projected = \
                                get_projected_data_model(data_test=mua_test_new,
                                                         test_time=(start_space_train,
                                                                    end_space_train),
                                                         trial_type_test=trial_type_test,
                                                         condition_test=behavior_distractor,
                                                         model=clf)
                            ...
                            for seed_sh in range(10):
                                ...
                                new_labels_sh = np.copy(mua_test_new[behavior_distractor].values)
            ...
            cv_accuracy_df.to_csv(os.path.join(PATH_SAVE,
                                               f'{SESSION}_{probe}_'
                                               f'cv_accuracy_{behavior_train}_LDA_dist_distractors'
                                               f'.csv'),
                                  index=False)
            ...
            cues = ['SC1', 'SC2', 'SC3']
            for cue in cues:
                figure, axis = plt.subplots(1, 1, figsize=(5, 5), sharey='row')
                hue_order = [False, True]
                sns.boxplot(
                    data=cv_accuracy_df[(cv_accuracy_df.event_train == cue) &
                                        (cv_accuracy_df.behavior == f'{cue}_dir')
                                        ],
                    x='trial_type', y='diff_distance', hue='shuffled', whis=[5, 95],
                ...
                axis.legend_.remove()
                axis.set_title(f'Encoding distractor {cue}_dir')
                axis.invert_yaxis()
                figure.suptitle(f'{SESSION}-{probe} training at: {cue}')
                figure.tight_layout()
                sns.despine(offset=10, trim=True)
                figure.savefig(os.path.join(PATH_FIGURES,
                                            f'{SESSION}_{probe}_'
                                            f'{cue}_diff_dist_distractors.png'),
                               bbox_inches='tight', dpi=300)
                plt.close(figure)
            io.logger.info(f'{SESSION}-{probe}-finished')
