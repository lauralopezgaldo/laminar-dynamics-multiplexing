...
cues and checks again the measure. It uses the distance metric as the other script.
"""
        ...
        for probe in probes:
            ...
            for event_train in events_train:
                ...
                for bt in range(100):
                    ...
                    mua_train, mua_test = \
                        split_train_test(mua_xr=mua_site.sel(trial_type=trial_type_correct),
                    ...
                    for event_test in events_test:
                        trial_type_test = int(event_test[-1])
                        if event_test == event_train:
                            continue
                        else:

                            # 2.2 - Set the time-window to test the LDA model
                            start_space_test, end_space_test = \
                                get_time_window(mua=mua_site, event_name=event_test,
                                                window_duration=.3,
                                                alignment='mid_cue')
                            # get the trials for the test - new trial type
                            mua_test_new = mua_site.sel(trial_type=trial_type_test)

                            # project in the original model with a different time-window
                            mua_test_projected = \
                                get_projected_data_model(data_test=mua_test_new,
                                                         test_time=(start_space_test,
                                                                    end_space_test),
                                                         trial_type_test=trial_type_test,
                                                         condition_test=behavior_train,
                                                         model=clf)

                            # 2.5 - Compute the distance to centroid for each trial type -
                            # real and shuffled
                            diff_avg_distances, accuracy = \
                                get_distance_to_centroid(trials_train=mua_train_projected,
                                                         trials_test=mua_test_projected,
                                                         labels_train=mua_train[
                                                             behavior_train].values,
                                                         labels_test=mua_test_new[
                                                             behavior_train].values)
                            ...
                            for seed_sh in range(10):
                                # shuffle the labels
                                np.random.seed(seed_sh)
                                new_labels_sh = np.copy(mua_test_new[behavior_train].values)
                                np.random.shuffle(new_labels_sh)
                                # compute the distance
                                diff_avg_distances_sh, accuracy_sh = \
                                    get_distance_to_centroid(trials_train=mua_train_projected,
            ...
            cv_accuracy_df.to_csv(os.path.join(PATH_SAVE,
                                               f'{SESSION}_{probe}_'
                                               f'cv_accuracy_{behavior_train}_LDA_SCs_dir.csv'),
                                  index=False)
            ...
            axis.set_title(f'{measure} for dir'
            ...
            figure.savefig(f'{PATH_FIGURES}/{SESSION}_{probe}_{measure}_dir_train_'
                           f'{train_split_fraction}_SC2.png',
                           dpi=300, bbox_inches='tight')

            # plot the three cues together in the same figure
            figure2, axis2 = plt.subplots(1, 3, figsize=(15, 7), sharey='row')
            x_events_train = ['SC1', 'SC2', 'SC3']
            measure = 'diff_distance'
            x_order = ['SC1', 'SC2', 'SC3']

            for x_event_train, ax_i in zip(x_events_train, axis2):
                hue_order = [False, True]
                sns.boxplot(
                    data=cv_accuracy_df[cv_accuracy_df.event_train == x_event_train],
                    x='event_test', y='diff_distance', hue='shuffled', whis=[5, 95],
                    palette=palette,
                    dodge=True, order=x_order,
                    hue_order=hue_order,
                    ...
                    medianprops=dict(color='black'),
                    whiskerprops=dict(color='black'),
                    capprops=dict(color='black'), ax=ax_i
                )
                ...
                sns.stripplot(
                    ...
                    palette=palette,  # Avoid the ValueError
                    alpha=.1, linewidth=0.1,
                    marker='o', zorder=1, edgecolor='gray', ax=ax_i
                )
                ax_i.legend_.remove()
                ax_i.invert_yaxis()
                sns.despine(offset=10, trim=True, ax=ax_i)
                ax_i.set_title(f'train in {x_event_train} ({train_split_fraction}%)')

            figure2.suptitle(f'{SESSION} - {probe}')
            figure2.tight_layout()

            figure2.savefig(f'{PATH_FIGURES}/{SESSION}_{probe}_{measure}_dir_train_'
                            f'{train_split_fraction}_all_SCs.png',
                            dpi=300, bbox_inches='tight')

            # create a general figure just with the distances for generalization and not (same cue)
            figure3, axis3 = plt.subplots(1, 1, figsize=(5, 5))
            x_order = [True, False]
            sns.boxplot(
                data=cv_accuracy_df,
                x='same_cue', y='diff_distance', hue='shuffled', whis=[5, 95],
                palette=palette,
                dodge=True, order=x_order,
                hue_order=hue_order,
                showfliers=False,
                ...
                medianprops=dict(color='black'),
                whiskerprops=dict(color='black'),
                capprops=dict(color='black'), ax=axis3
            )
            sns.stripplot(
                data=cv_accuracy_df,
                x='same_cue', y='diff_distance', hue='shuffled',
                dodge=True, order=x_order,
                hue_order=hue_order,
                jitter=True,
                palette=palette,  # Avoid the ValueError
                alpha=.1, linewidth=0.1,
                marker='o', zorder=1, edgecolor='gray', ax=axis3
            )
            axis3.legend_.remove()
            axis3.invert_yaxis()
            sns.despine(offset=10, trim=True, ax=axis3)

            figure3.suptitle(f'{SESSION} - {probe} - generalization')
            figure3.tight_layout()

            figure3.savefig(f'{PATH_FIGURES}/{SESSION}_{probe}_{measure}_dir_train_'
                            f'{train_split_fraction}_same_cue_vs_gen.png',
                            dpi=300, bbox_inches='tight')

            # plot the three cues together in the same figure
            figure4, axis4 = plt.subplots(1, 3, figsize=(15, 7), sharey='row')
            x_events_train = ['SC1', 'SC2', 'SC3']
            measure = 'diff_distance'
            x_order = [True, False]

            for x_event_train, ax_i in zip(x_events_train, axis4):
                hue_order = [False, True]
                sns.boxplot(
                    data=cv_accuracy_df[cv_accuracy_df.event_train == x_event_train],
                    x='same_cue', y='diff_distance', hue='shuffled', whis=[5, 95],
            ...
            figure4.suptitle(f'{SESSION} - {probe}')
            figure4.tight_layout()

            figure4.savefig(f'{PATH_FIGURES}/{SESSION}_{probe}_{measure}_dirSCs_'
                            f'{train_split_fraction}_generalization_per_cue.png',
                            dpi=300, bbox_inches='tight')

            plt.close('all')
            io.logger.info(f'{SESSION} probe {probe} saved.')
