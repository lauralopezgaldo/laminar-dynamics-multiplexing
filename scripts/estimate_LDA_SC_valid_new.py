...
if __name__ == "__main__":
    ...
    SESSIONS = ['t150327002-t150327003']
    ...
    # PATH_EXCEL = "C:/Users/laura/OneDrive/Escritorio/data_local/"
    ...
    for SESSION in SESSIONS:
        PATH = server + \
            ...
            SESSION + '/'
        # PATH = PATH_EXCEL + '/Preprocessed_data/'
        PATH_DATA = server + \
            '/comco/lopez.l/Electrophysiology/ephy_laminar_MUA/Results/Full_trial/' \
            'data/' + SESSION + '/'
        PATH_SAVE = server + \
        ...
        for probe in probes:
            ...
            for bt in range(50):
                ##################
                # BUILD THE SPACE AT SCi - TRAIN AND TEST HERE.
                ...
                # get the new labels for the train
                mua_train_validity, y_train_validity = \
                    get_test_data_valid_at_scs(data_test=mua_train, events_scs=[event_train],
                                               trial_type_test=trial_types,
                                               condition_test=behavior_train)

                mua_test_validity, y_test_validity = \
                    get_test_data_valid_at_scs(data_test=mua_test, events_scs=[event_train],
                                               trial_type_test=trial_types,
                                               condition_test=behavior_train)

                # correct for the imbalance classes - always: invalid = 2*valid
                for seed_v in range(5):
                    ...
                    # 2.2 - From the balanced train: build an LDA model
                    clf_validity = LDA()
                    clf_validity.fit(mua_train_validity_balanced, y_train_validity_balanced)

                    # project the data onto the model (train and test)
                    mua_train_validity_proj = clf_validity.transform(mua_train_validity_balanced)
                    mua_test_validity_proj = clf_validity.transform(mua_test_validity_balanced)

                    # check the distance to the train centroids, in this case just VALID and INVALID
                    diff_avg_distances, accuracy = \
                        get_distance_to_centroid(trials_train=mua_train_validity_proj,
                                                 trials_test=mua_test_validity_proj,
                    ...
                    cv_accuracy.append({'event': event_train, 'bootstrap': bt,
                                        'accuracy': accuracy, 'same_cue': True,
                    ...
                    for seed_sh in range(5):
                        ...
                        new_labels_sh = np.copy(y_test_validity_balanced)
                        ...
                        cv_accuracy.append({'event': event_train, 'bootstrap': bt,
                                            'accuracy': accuracy_sh, 'same_cue': True,
                                            'diff_distance': diff_avg_distances_sh,
                                            'shuffled': True})

                    # Without rebuilding the model: check the projections in other cues
                    for event_test in events_test:
                        mua_test_validity, y_test_validity = \
                            get_test_data_valid_at_scs(data_test=mua_site, events_scs=[event_test],
                                                       trial_type_test=trial_types,
                                                       condition_test=behavior_train)

                        # correct for the imbalance classes - always: invalid = 2*valid
                        for seed_vt in range(5):
                            mua_test_validity_balanced, y_test_validity_balanced, _, _ = \
                                split_train_test_array(mua_test_validity, y_test_validity,
                                                       seed=seed_vt,
                                                       percent_train=1)
                            mua_test_validity_proj = \
                                clf_validity.transform(mua_test_validity_balanced)

                            # check the distance to the train centroids, in this case just VALID and
                            # INVALID
                            diff_avg_distances, accuracy = \
                                get_distance_to_centroid(trials_train=mua_train_validity_proj,
                                                         trials_test=mua_test_validity_proj,
                                                         ...
                                                         labels_test=y_test_validity_balanced)
                            # get the single trial accuracy
                            cv_accuracy.append({'event': event_test, 'bootstrap': bt,
                                                'accuracy': accuracy, 'same_cue': False,
                                                'diff_distance': diff_avg_distances,
                                                'shuffled': False})
            ...
            cv_accuracy_df.to_csv(os.path.join(PATH_SAVE,
                                               f'{SESSION}_{probe}_'
                                               f'cv_accuracy_{behavior_train}_LDA_SC_valid_'
                                               f'80train.csv'),
                                  index=False)
            ...
            sns.stripplot(
                ...
                hue_order=hue_order,
                jitter=True,
                palette=palette_validity,  # Avoid the ValueError
            ...
            figure.savefig(f'{PATH_FIGURES}/LDA_color_condition_{SESSION}_{probe}_'
                           f'train_{train_split_fraction}_difference_distance.svg')
            plt.close(figure)
            io.logger.info(f'{SESSION}-{probe}-finised')
